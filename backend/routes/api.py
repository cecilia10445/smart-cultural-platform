...
_g_context = contextvars.ContextVar("g_context")


class RequestContext:
    """Small compatibility view used while handlers are split by domain."""

    def __init__(self, raw_request, body):
        self.headers = raw_request.headers
        self.args = raw_request.query_params
        self.json = body
        self._body = body
...
class ContextProxy:
    ...
    def __getattr__(self, name):
        return getattr(self._storage.get(), name)

    def __setattr__(self, name, value):
        if name == "_storage":
            object.__setattr__(self, name, value)
        else:
            setattr(self._storage.get(), name, value)


request = ContextProxy(_request_context)
g = ContextProxy(_g_context)
...
def send_file(path, mimetype=None, as_attachment=False, download_name=None, max_age=None):
    headers = {}
...
def _fastapi_path(path):
    return (path.replace("<path:filename>", "{filename:path}")
                .replace("<run_id>", "{run_id}"))


def _dispatch(handler, raw_request, body=None, run_id=None, filename=None):
    request_token = _request_context.set(RequestContext(raw_request, body))
    g_token = _g_context.set(SimpleNamespace())
    try:
        values = {"run_id": run_id, "filename": filename}
        kwargs = {name: values[name] for name in inspect.signature(handler).parameters if values[name] is not None}
        ...
        _g_context.reset(g_token)
        _request_context.reset(request_token)


class LegacyDecorator:
    """No-op decorator retained solely while legacy handler bodies move out."""
...
def generate_cultural_product_api():
    ...
    if raw_idempotency_key is not None and (
        ...
        return api_error("INVALID_IDEMPOTENCY_KEY", "Idempotency-Key must be a non-empty value up to 128 characters.", 400, False, False, "Idempotency-Key")
    ...
    try:
        ...
        existing_attempt = tracker.start()
    ...
    if existing_attempt:
        if existing_attempt.get("brief_sha256") != tracker.brief_sha256(brief):
            return api_error("IDEMPOTENCY_KEY_CONFLICT", "Idempotency-Key was already used for a different generation request.", 409, False)
        status = existing_attempt.get("status")
        if status == "RUNNING":
            return api_error("GENERATION_IN_PROGRESS", "Generation is already in progress for this Idempotency-Key.", 409, True)
        if status == "SUCCEEDED":
            generation_log_id = existing_attempt.get("generation_log_id")
            rows = mysql_service.execute_query(
                "SELECT id,response_json FROM generation_logs WHERE id=%s AND user_id=%s LIMIT 1",
                (generation_log_id, user_info["user_id"]),
                max_retries=0,
            )
            row = rows[0] if isinstance(rows, list) and rows and isinstance(rows[0], dict) else None
            stored_response = row.get("response_json") if row else None
            ...
            if isinstance(stored_response, dict):
                replay = {**stored_response, "log_id": generation_log_id, "request_id": existing_attempt.get("request_id")}
                return jsonify(replay)
            return api_error("IDEMPOTENCY_RESULT_UNAVAILABLE", "The completed generation result is unavailable.", 503, True, True)
        return api_error(
            existing_attempt.get("error_code") or "GENERATION_PREVIOUSLY_FAILED",
            "The previous generation for this Idempotency-Key failed. Submit a new request to retry.",
            409,
            False,
        )
    ...
    try:
        if hasattr(aigc_service, "generate_cultural_product_text_with_evidence"):
            ...
            text_result = {
                **text_result,
                "factual_background": {"text": "；".join(brief["confirmed_facts"]), "source_type": "user" if brief["confirmed_facts"] else "none"},
                "used_source_ids": [],
                "evidence_status": text_result.get(
    			"evidence_status",
    			"creative_only"
			),
            }
    ...
    factual = factual_background(
        brief,
        text_result["factual_background"]["text"] if isinstance(text_result["factual_background"], dict) else text_result["factual_background"],
        verified_sources,
