...
def test_same_orchestrator_reaches_final_seal_comparable(monkeypatch,tmp_path):
  frozen={'status':'grounded','sources':[{'source_id':'s1'}]}; monkeypatch.setattr(o,'freeze_evidence',lambda _:frozen); monkeypatch.setattr(o,'run_baseline',lambda *a:(_out('A'),{'requests':1,'latency_ms':1})); deps=SimpleNamespace(loaded_skill_body='x'); monkeypatch.setattr(o,'run_guided_plan',lambda *a:(deps,SimpleNamespace(model_dump=lambda:{'selected_text_skill_id':'x'}),{'requests':2,'tool_trajectory':[{'tool':'retrieve_cultural_sources'},{'tool':'load_generation_skill'}]})); monkeypatch.setattr(o,'run_guided_final',lambda *a:(_out('B'),{'requests':1,'latency_ms':1}))
  def pf(_g,p):
    rows=[]
...
def test_same_orchestrator_position_bias(monkeypatch,tmp_path):
  frozen={'status':'grounded','sources':[{'source_id':'s1'}]}; monkeypatch.setattr(o,'freeze_evidence',lambda _:frozen); monkeypatch.setattr(o,'run_baseline',lambda *a:(_out('A'),{'requests':1})); deps=SimpleNamespace(loaded_skill_body='x'); monkeypatch.setattr(o,'run_guided_plan',lambda *a:(deps,SimpleNamespace(model_dump=lambda:{}),{'requests':2,'tool_trajectory':[]})); monkeypatch.setattr(o,'run_guided_final',lambda *a:(_out('B'),{'requests':1}))
  def pf(_g,p):
