...
传统生成式应用通常是“一次输入 → 一次结果”。本项目将持续 Conversation、可切换的设计目标、版本化设计产物和有副作用的业务命令分层建模：用户可以先研究文化意象、比较方案或讨论修改；只有在需要保存正式设计稿、生成图片或归档设计时，系统才进入明确的 Human-in-the-loop（人在回路）确认路径。
